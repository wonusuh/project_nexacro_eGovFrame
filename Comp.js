...
pForm.gfn_openMonthCal = function (obj) {
    ...
    if (this.gfn_isNull(this.components[compid])) {
        ...
        objCalPopupDiv.addEventHandler(
            "oncloseup",
            this.gfn_monthCalClose,
            this
        );
    } else {
...
pForm.gfn_monthCalClose = function (obj) {
    if (!this.gfn_isNull(obj.callbackfunc)) {
        this.lookupFunc(obj.callbackfunc).call(obj.calId);
    }
};

pForm.gfn_openMultiCombo = function (obj) {
    if (
        this.gfn_isNull(obj.u_innerdataset) ||
        this.gfn_isNull(obj.u_codecolumn) ||
        this.gfn_isNull(obj.u_datacolumn)
    ) {
    ...
    if (this.gfn_isNull(objComboDs)) {
        ...
        if (objDs.getRowCount() < displayCnt)
            displayCnt = objDs.getRowCount();
    ...
    if (this.gfn_isNull(objGrid)) {
        ...
        oGrd.addEventHandler(
            "onheadclick",
            this.gfn_grid_onheadclick,
            this
        );
        ...
        if (nComboSize > nGridSize) {
            ...
            oGrd.setRealColSize(
                "body",
                2,
                nComboSize - nGridCol0 - nGridCol1 - nScrollSize
            );
            objPdv.set_width(nComboSize);
        }
...
pForm.gfn_getCommCode = function () {
    ...
    for (var i = 0; i < arrComm.length; i++) {
        ...
        //       sCompId = this.components[sCompId];
        ...
        if (sCompId instanceof nexacro.Dataset) {
            ...
            sCompId.set_codecolumn("CODE");
            sCompId.set_datacolumn("NAME");
        }

        sCodeType = strDs.substr(strDs.length - 4, 4); //ds_C003...
        objApp.gds_commCode.filter("TYPE =='" + sCodeType + "'");
        ...
        if (!this.gfn_isNull(sType)) {
            if (sType != 2) {
                ...
                objDs.setColumn(
                    0,
                    "NAME",
                    sType == 0 ? "ALL" : "please selected"
                );
            }
        }

        if (
            sCompId instanceof nexacro.Combo ||
            sCompId instanceof nexacro.ListBox ||
            sCompId instanceof nexacro.Radio
        ) {
            sCompId.set_index(0);
        }
    }
...
pForm.gfn_getDLLPath = function (sFileName) {
    ...
    if (xadl.indexOf("file://") != -1) {
        ...
        strpath =
            nexacro.replaceAll(
                xadl.substring(0, xadl.lastIndexOf("/")),
                "/",
                "\\"
            ) +
            "\\dll\\" +
    ...
    // 웹 접속
        ...
        strpath = system.convertRealPath(
            "%USERAPP%\\Component\\" + sFileName
        );
    }
